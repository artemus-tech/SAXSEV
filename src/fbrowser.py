...
class MainWidget:
    ...
    def init_presets(self):
        ...
        self.indicatrix = None

        #reset flags
        self.vfdf_path_for_iter = ""
        ...
        self.loopNumberCounter=0
        self.currentIteration = 0
        self.currentiterattionNumber = 0
        self.tmp_reduce_path = ""
        self.old_iter_reduce_path =""
        # reset text fields
        ...
        ict.set_location(self.iteration_number_entry,"")
        ict.set_location(self.iter_numb_to_avg_entry,"")
        ict.set_location(self.loop_number_entry,"")
        ...
        self.iteration_path_list = []
        self.iter_numb_to_avg = 4
        self.vfdf_iter_avg = []
    ...
    def make_menubar(self,frame): 
        ...
        # Load OriginIntensity&Scattering vector
        #self.Originindi_path_entry, _ = lm_main.make_file_entry(menubar1, pos=1, lbl_text="OriginIndicatrix", cmd=self.get_Originindicatrix)
        
        
        # Scattering vecotr/angle user choise
        #self.to_scat_vect_chb, self.to_scat_vect = lm_main.make_checkbox(menubar1, pos=3, lbl_text="Convert THETTA to q automatically",cmd=self.get_q)
        
        # Button for Loading Rg-Grid
        self.grid_path_entry, _ = lm_main.make_file_entry(menubar1, pos=4, lbl_text="Rg-Grid", cmd=self.upload_datagrid, v_pad=(4,0)) 
        lm_main.make_btn_grid(menubar1, row=5, column=2, btn_text="Generate", cmd=self.generate_datagrid, btn_width=10, btn_padx=1, btn_pady=0)
        # Textbox to set Anisometric factor
        self.anisometer_factor_entry = lm_main.make_entry_grid( menubar1, pos=6, lbl_text="Anisometric factor") 
        # create shape selector for distribution estimation
        self.shapes_combo = lm_main.make_combo_grid(menubar1, pos=7, lbl_text="Particle shape", options=cfg.shapes)
        ...
        self.alpha_opt_entered_chb, self.alpha_opt_entered = lm_main.make_checkbox(menubar2, pos=6, lbl_text="Enter alpha regulizer",cmd=self.__switch_alpha_setup )
        self.alpha_setup_entry = lm_main.make_entry_grid( menubar2, pos=7, lbl_text="Alpha:", state=DISABLED) 



        # Textbox to set Volume Fraction
        self.volume_fraction_entry = lm_main.make_entry_grid( menubar2, pos=8, lbl_text="Volume Fraction") 

        lm_main.make_btn_grid(menubar2, row=8, column=2, btn_text="Zero-Iteration", cmd=self.calc_zero_iter, btn_width=10,btn_padx=1,btn_pady=0)

        # Textbox to set Anisometric factor
        self.iteration_number_entry = lm_main.make_entry_grid( menubar2, pos=9, lbl_text="Iterations number") 
        lm_main.make_btn_grid(menubar2, row=9, column=2, btn_text="Run Iterations", cmd=self.Run, btn_width=10, btn_padx=1, btn_pady=0)
        # second section for Svergun params and VFDF operation
        self.iter_numb_to_avg_entry = lm_main.make_entry_grid( menubar2, pos=10, lbl_text="VFDF number") 
        #calculate nhnnn
        lm_main.make_btn_grid(menubar2, row=10, column=2, btn_text="Average", cmd=self.average, btn_width=10, btn_padx=1, btn_pady=0)


        self.loop_number_entry = lm_main.make_entry_grid( menubar2, pos=11, lbl_text="Loops-number") 
        lm_main.make_btn_grid(menubar2, row=11, column=2, btn_text="Run Loop", cmd=self.RunLoop, btn_width=10, btn_padx=1, btn_pady=0)
        ...
        menubar4 = lm_main.make_lbl_frame(frame,lbl_frame_text="Iteration method")

        lm_main.make_file_entry(menubar4, pos=2, lbl_text="VFDF-instance", cmd=print(1), v_pad=0)



        # pack main dialog       
        menubar1.pack(side=LEFT, fill=BOTH ,expand = False, padx = 5, pady = 5)    
        menubar2.pack(side=LEFT, fill=BOTH, expand = False, padx = 5, pady = 5)    
        menubar3.pack(side=LEFT, fill=BOTH, expand = False, padx = 5, pady = 5)
        menubar4.pack(side=LEFT, fill=BOTH, expand = False, padx = 5, pady = 5)
        return
    ...
    def RunLoop(self):
        self.loopNumberCounter = ict.get_int_from_entry(self.loop_number_entry)
        print("===========================RunLoop|varname===================================")
        print(self.loopNumberCounter)
        print(self.currentIteration)
        print(self.iter_numb_to_avg)
        print("===========================RunLoop|varname===================================")
        for i in range(int(self.loopNumberCounter)):
            #if self.currentIteration / self.iter_numb_to_avg:
            #    self.average()
            self.Run()
            self.average()
    ...
    def average(self):
        ...
        print("===========================Debug===================================")
        self.iter_numb_to_avg = ict.get_int_from_entry(self.iter_numb_to_avg_entry)
        print("iter_numb_to_avg:"+str(self.iter_numb_to_avg))
        print("===========================Debug===================================")

        print(self.iteration_path_list)

        print("===========================Debug|iteration_path_list===================================")
        self.varname(self.iteration_path_list)
        print("===========================Debug===================================")

                   
        iterationsNumb = np.arange(self.currentIteration-self.iter_numb_to_avg+1, self.currentIteration+1)

        print("===========================Debug|iterationsNumbers===================================")
        print(iterationsNumb)
        print("===========================Debug|iterationsNumbers===================================")
        ...
        #avg2 = None
        print(self.cwd)
        print(cfg.paths["output_data_path"])
        print(os.getcwd())
        out = np.zeros((len(self.rg), 9 ))
        print(np.shape(out))
        print(out)
        for i in iterationsNumb:
            
            tmp = str(i)
            print("=====================TEMP=====================================")
            print("AVEREGE: "+tmp)
            print("=====================TEMP=====================================")
            if tmp in os.path.basename(self.iteration_path_list[i])[0:3]:
                print(self.iteration_path_list[i])
                out += np.loadtxt(self.iteration_path_list[i])

        print("===========================Debug===================================")
        print("currentIteration:"+str(self.currentIteration))
        print("===========================Debug===================================")
        self.calc_strfactor_and_reduce_intensity_from_vfdf(out/self.iter_numb_to_avg,self.currentIteration+1)
        ...
        print(self.currentiterattionNumber)
    ...
    def Run(self):               
        ...
        if self.iterationCounter==0:
            self.iterationCounter = ict.get_int_from_entry(self.iteration_number_entry)
            self.currentiterattionNumber = 1
        else:
            self.currentiterattionNumber = self.currentiterattionNumber+1
            self.iterationCounter = ict.get_int_from_entry(self.iteration_number_entry)
            
        if self.alpha_opt_entered.get()==1:        
            self.alpha_setup = ict.get_float_from_entry(self.alpha_setup_entry)
        else:
            self.alpha_setup = ict.get_float_from_entry(self.alpha_result_entry)
                
        self.choise = ict.get_string_from_combo(self.shapes_combo)

           
   
        for i in range(self.currentiterattionNumber,self.currentiterattionNumber + self.iterationCounter):
            print("==============================Iteration Step=============================")
            print("IterationNumber:"+str(i))
            print("==============================Iteration Step=============================")

            print("=======================Temprary recuce path================================")
            print(self.tmp_reduce_path)
            print("=======================Temprary recuce path================================")


            pathtoreduce = self.tmp_reduce_path.replace('./',cfg.paths["output_data_path"]+'/logExp_ellipsoid_1.0/')                                         
            iterationIntens = np.loadtxt(pathtoreduce)
            self.old_iter_reduce_path=pathtoreduce


            K = VFDF(iterationIntens[:,1],iterationIntens[:,0], self.rg, np.sqrt(iterationIntens[:,1]), self.alpha_setup)
            ...
            self.alpha_optimum = self.alpha_setup
            ...
            self.__calc_zero_iter_by_alpha()
                
        self.currentiterattionNumber = self.currentiterattionNumber + self.iterationCounter
        return 
    ...
    def get_indicatrix(self):
        ...
        if self.indi_path:
            ...
            if self.indi_is_uploaded:
                ...
                self.scattering_Originintens = M[:,1]
    ...
    def get_Originindicatrix(self):
        """start method for uploading indicatrix vector"""
        self.Originindi_path = askopenfilename(filetypes = [("allfiles","*"),("pythonfiles","*.py")], initialdir=cfg.paths["input_data_path"])
        # clear list of missing vectors
        self.indi_err_message[:]=[]
        if self.Originindi_path:
            M = np.loadtxt(self.Originindi_path)
            # set status of data uploading
            self.Originindi_is_uploaded = True
            self.__check_indicatrix_format(M)
            if self.Originindi_is_uploaded:
                        
                self.scattering_Originintens = M[:,1]
                
                ict.set_location(self.Originindi_path_entry, self.Originindi_path)
            else:
                messagebox.showwarning(
                    "Upload indicatrix",
                    "\n".join(self.indi_err_message)
            ...
            # save record to log
            ict.save_log(cfg.paths["log_file_path"],"Upload saxs indicatrix " + self.Originindi_path)     
        return
    ...
    def __switch_alpha_setup(self, evt):
        """Disable or Enable vfdf-load button"""
        print(self.alpha_opt_entered.get())           
        if self.alpha_opt_entered.get() < 1:
            self.alpha_setup_entry.config(state = NORMAL)
        else:
            self.alpha_setup_entry.config(state = DISABLED)
    ...
    def __generate_vfdf_ivfdf_files(self):
        ...
        if self.alpha_opt_entered.get()==1:
            K = VFDF(self.scattering_intens, self.q, self.rg, self.scattering_intens_ambiguity, self.alpha_setup)
            self.alpha_optimum = self.alpha_setup
            self.alpha_result_entry.configure(state=NORMAL)
            self.alpha_result_entry.delete(0, END)
            self.alpha_result_entry.insert(0,self.alpha_optimum)
    ...
    def eval_vfdf(self):
        ...
        self.vfdf_eval_input_is_uploaded=True
        self.anisometer_factor = ict.get_float_from_entry(self.anisometer_factor_entry)
        self.iterationCounter = ict.get_float_from_entry(self.iteration_number_entry)
        self.alpha_setup = ict.get_float_from_entry(self.alpha_setup_entry)
        self.choise = ict.get_string_from_combo(self.shapes_combo)
    ...
    def view_pc(self):
        ...
        if self.pc_file_name:
            ...
            fig.canvas.get_tk_widget().focus_force()
            # to set window title
            fig.canvas.manager.set_window_title('Percept Criteria')
    ...
    def __get_vfdf_by_alpha(self):
        ...
        if vfdf_auto_selected_filename:
            if self.__validate_view_params(vfdf_auto_selected_filename):
                ...
                """
                #self.calc_strfactor_and_reduce_intensity_from_vfdf(vfdf_matrix,self.currentIteration)
    ...
    def calc_strfactor_and_reduce_intensity_from_vfdf(self,vfdf_matrix,iternum): 
        DT = str(datetime.datetime.now()).replace(":", "_")  
        ...
        if iternum==0:
            teorNAME ="./vrija"+DT+"/"+"0"+"teorIntens"+"_alfa="+str(self.alpha_optimum) + '.txt'
        else:
            teorNAME ="./vrija"+DT+"/"+str(iternum)+"teorIntens"+"_alfa="+str(self.alpha_optimum) + '.txt'

        print(teorNAME)
        np.savetxt( teorNAME,np.array([self.q,np.array(Iteor)]).T)

        if iternum==0:
            Distr_Name = "./vrija"+DT+"/"+"0"+"selected_Rg_VFDF_VFDFPOS_R_NDR"+"_alfa="+str(self.alpha_optimum) + '.txt'
        else:
            Distr_Name = "./vrija"+DT+"/"+str(iternum)+"selected_Rg_VFDF_VFDFPOS_R_NDR"+"_alfa="+str(self.alpha_optimum) + '.txt'
        ...
        if iternum==0:
            Copy_Distr_Name = "./vrija"+DT+"/"+"0"+self.vfdf_path_for_iter
        else:
            Copy_Distr_Name = "./vrija"+DT+"/"+str(iternum)+self.vfdf_path_for_iter

        #__calc_zero_iter_by_alpha
        print("===============================self.vfdf_path_for_iter===================================")
        print(self.vfdf_path_for_iter)
        print("===============================self.vfdf_path_for_iter===================================")
        #np.savetxt( Distr_Name,vfdf_matrix)

        # add to list iterations pdf path
        self.iteration_path_list.append(Distr_Name)
        np.savetxt( Copy_Distr_Name,vfdf_matrix)
        np.savetxt( Distr_Name,vfdf_matrix)
       
        src_intens = self.scattering_Originintens 

        vrij.NC = ict.get_float_from_entry(self.volume_fraction_entry)
        ...
        if iternum==0:   
            mono="./vrija"+DT+"/"+"0"+"no_volume_vrija_"+"NC="+str(vrij.NC)+"_alfa="+str(self.alpha_optimum) + '.txt'

        else:
            mono="./vrija"+DT+"/"+str(iternum)+"no_volume_vrija_"+"NC="+str(vrij.NC)+"_alfa="+str(self.alpha_optimum) + '.txt'

        print(mono)
        
        np.savetxt( mono,np.array([self.q,np.array(vrija),np.sqrt(vrija),np.sqrt(vrija)]).T)
        ...
        if iternum==0:
            OutstrFactor ="./vrija"+DT+"/"+"0"+"strFactor_"+"NC="+str(vrij.NC)+"_alfa="+str(self.alpha_optimum) + '.txt'
        else:
            OutstrFactor ="./vrija"+DT+"/"+str(iternum)+"strFactor_"+"NC="+str(vrij.NC)+"_alfa="+str(self.alpha_optimum) + '.txt'
        #print(OutstrFactor)  
        ...
        if iternum==0:
            redScattInt="./vrija"+DT+"/"+"0"+"Q_reduce_scattering_intens_"+"NC="+str(vrij.NC)+"_alfa="+str(self.alpha_optimum) + '.txt'
        else:        
            redScattInt="./vrija"+DT+"/"+str(iternum)+"Q_reduce_scattering_intens_"+"NC="+str(vrij.NC)+"_alfa="+str(self.alpha_optimum) + '.txt'
        self.tmp_reduce_path = redScattInt #"./vrija"+DT+"/"+str(iternum)+"Q_reduce_scattering_intens_"+"NC="+str(vrij.NC)+"_alfa="+str(self.alpha_optimum) + '.txt'

        print(self.tmp_reduce_path)
        
        if iternum>=1:
            m = np.loadtxt(self.old_iter_reduce_path)
            np.savetxt("./vrija"+DT+"/"+str(iternum-1)+"Q_reduce_scattering_intens_"+"NC="+str(vrij.NC)+"_alfa="+str(self.alpha_optimum) + '.txt', m)
    ...
    def __calc_zero_iter_by_alpha(self):
        """find vfdf-matrix by alpha optimum"""
        os.chdir(self.vfdf_path)      
        ...
        if vfdf_auto_selected_filename:
            self.vfdf_path_for_iter=vfdf_auto_selected_filename
            if self.__validate_view_params(vfdf_auto_selected_filename):
    ...
    def upload_vfdf(self):
        ...
        if is_manual:
            ...
            vfdf_matrix = np.loadtxt(self.vfdf_manual_selected_path)            
            print(np.loadtxt(self.vfdf_manual_selected_path))
            #self.calc_strfactor_and_reduce_intensity_from_vfdf(vfdf_matrix)
            os.chdir(cfg.paths["output_data_path"]+"/logExp_ellipsoid_"+str(self.anisometer_factor))
            print(cfg.paths["output_data_path"]+"/logExp_ellipsoid_"+str(self.anisometer_factor))
            self.calc_strfactor_and_reduce_intensity_from_vfdf(vfdf_matrix,self.currentiterattionNumber)
            os.chdir(self.cwd)
            #self.calc_strfactor_and_reduce_intensity_from_vfdf(vfdf_matrix,self.currentiterattionNumber)
